...
class Seq2SeqTrainer(object):
    def __init__(self, analyses_processor, model_configuration, utils):
        ...
        build_validation_model = BuildValidationModel(model_configuration, analyses_processor.vocabulary, analyses_processor.inverse_vocabulary)
    ...
    def __create_train_variables(self):
        with self.__train_model.graph.as_default():
            with tf.variable_scope('metrics'):
                ...
                return loss, learning_rate, global_step, accuracy, update_step, merged_summary_op, stat_ops

    def __validate_model(self, validation_dataset, validation_saver, train_saver, r_global_step, epoch_id, total_train_batches):
        print('\tValidating latest train model...')

        print('\tRestoring from train model...')
        ...
        total_validation_batches = validation_dataset.source_input_examples.shape[0] // self.__config.train_batch_size
        print('\t#{total validation batches}:', total_validation_batches)

        if validation_dataset.source_input_examples.shape[0] < self.__config.train_batch_size:
            raise ValueError('Validation dataset is too small: validation_dataset.source_input_examples.shape[0]='+str(validation_dataset.source_input_examples.shape[0])+'<train_batch_size='+str(self.__config.train_batch_size))
        ...
        for validation_batch_id in range(total_validation_batches):
            source_input_batch = validation_dataset.source_input_examples[validation_batch_ptr:validation_batch_ptr + self.__config.train_batch_size]
            target_input_batch = validation_dataset.target_input_examples[validation_batch_ptr:validation_batch_ptr + self.__config.train_batch_size]
            target_output_batch = validation_dataset.target_output_examples[validation_batch_ptr:validation_batch_ptr + self.__config.train_batch_size]
            ...
            validation_batch_ptr += self.__config.train_batch_size
        ...
        if self.__global_minimum_loss is None:
            ...
            if avg_validation_loss < self.__global_minimum_loss:
                ...
                save_path = train_saver.save(self.__train_session, os.path.join(self.__config.model_directory, 'validation_checkpoints', self.__config.model_name))
    ...
    def __run_epochs(self, data_processor, sentence_dicts):
        loss, learning_rate, global_step, accuracy, update_step, merged_summary_op, stat_ops = self.__create_train_variables()
        ...
        if self.__config.train_continue_previous:
            print('\tRestoring train model from validation checkpoint...')
            with self.__train_model.graph.as_default():
        ...
        for epoch_id in range(self.__config.train_epochs):
            ...
            total_train_batches = train_dataset.source_input_examples.shape[0] // self.__config.train_batch_size
            ...
            for train_batch_id in range(total_train_batches):
                ...
                r_loss, r_learning_rate, r_global_step, r_accuracy, r_update_step, r_merged_summary_op, r_stat_ops = self.__train_session.run(
                [
                    loss,
                    learning_rate,
                    global_step,
            ...
            if (epoch_id+1) % self.__config.train_validation_modulo == 0:
                train_saver.save(self.__train_session, os.path.join(self.__config.model_directory, 'train_checkpoints', self.__config.model_name))
                self.__validate_model(validation_dataset, validation_saver, train_saver, r_global_step, epoch_id, total_train_batches)
